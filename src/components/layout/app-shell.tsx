...
import { filterNav, getPrimaryRole, NAV_BY_ROLE } from "@/config/navigation";
import { useAttendanceReconciliationRequests } from "@/lib/queries/attendance";
import { useAmendments, usePendingHRApprovals, useSubordinatesLeaves } from "@/lib/queries/leave";
      ...
      <aside className="hidden w-64 border-r lg:block">
        <div className="flex h-16 items-center gap-3 px-6">
          ...
          <div>
            ...
            <div className="text-muted-foreground text-xs">Role-aware dashboard</div>
          </div>
        </div>
        <Separator />
          ...
          {navItems.map((item) =>
            item.children ? (
              <div key={item.label} className="space-y-1">
                <div className="text-muted-foreground flex items-center gap-2 px-3 py-2 text-xs font-semibold uppercase">
                  {item.icon && <item.icon className="mr-1 size-4" />} {item.label}
                </div>
...
function Header({ pathname, title }: { pathname: string; title: string }) {
  ...
  const pendingAmendments =
    amendments?.filter(
      (amendment) => amendment.status === "PENDING" || amendment.status === "PROCESSING",
    ) ?? [];
  const amendmentCount = pendingAmendments.length;
  const pendingReconciliations =
    reconciliationRequests?.data?.filter((request) => request.status === "PENDING") ?? [];
  ...
  const subtitle = useMemo(() => {
    ...
    return pathname.replace("/dashboard/", "").replaceAll("-", " ").replaceAll("/", " · ");
  }, [pathname, title]);

  return (
    <header className="bg-background/80 sticky top-0 z-30 flex h-16 items-center justify-between border-b px-4 backdrop-blur lg:px-6">
      ...
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" className="flex items-center gap-2 px-2">
          ...
          <div className="hidden text-left lg:block">
            <p className="text-sm leading-tight font-semibold">{session?.user.name || "Guest"}</p>
            <p className="text-muted-foreground text-xs">{primaryRole || "Viewer"}</p>
          </div>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-56">
        <DropdownMenuLabel>
          <div className="space-y-1">
            <p className="text-sm leading-none font-semibold">{session?.user.name || "Guest"}</p>
      ...
      <DropdownMenuContent align="end" className="w-80">
        ...
        {isAdmin ? (
          <>
            <DropdownMenuItem asChild>
              <Link
                href="/dashboard/admin/leave?tab=approvals"
            ...
            <DropdownMenuItem asChild>
              <Link
                href="/dashboard/admin/leave?tab=amendments"
                className="flex w-full items-center justify-between"
              >
                <div className="flex items-center gap-2">
                  <Bell className="size-4" />
